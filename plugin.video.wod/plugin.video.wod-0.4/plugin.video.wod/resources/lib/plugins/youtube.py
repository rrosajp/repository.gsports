...
def swap_link(link):
    ...
    if 'plugin.video.youtube/playlist' in link:
        ...
        return (
            pl_base + link.split('/')[-2]
            if link.lower().endswith('/')
            else pl_base + link.split('/')[-1]
        )
    elif 'plugin.video.youtube/channel' in link:
        return (
            ch_base + link.split('/')[-2]
            if link.lower().endswith('/')
            else ch_base + link.split('/')[-1]
        )
    elif 'plugin.video.youtube/watch' in link or 'youtube.com/watch' in link:   
        return vid_base + link.split('=')[-1]
