...
class airtable(Plugin):
    ...
    def routes(self, plugin):
        ...
        def airtable_jen(table_info):
            ...
            if table_type in ["season", "show"]:
                match = at.search(
                    'category', f"{table_base}_{table_split[-1]}", view='Grid view'
                )
            else:
            ...
            for field in match:
                try:
                    ...
                    links = []
                    for k in keys:
                        if 'link' not in k: continue
                        elif 'link' in k and res[k] == '-' : continue
                        else: 
                            link = res[k]
                            if link == "-": continue
                            if "/live/" in link:
                                link = f"ffmpegdirect://{link}"
                            links.append(link)
                            if link.endswith(".json"): break

                    jen_data = {
                        "title": name,
                        "link": (links if len(links) > 1 else links[0])
                        if links
                        else "",
                        "thumbnail": thumbnail,
                        "fanart": fanart,
                        ...
                        if links
                        and (
                            links[0].endswith(".json")
                            or (
                                "youtube.com" in links[0]
                                and (
                                    "playlist" in links[0] or "channel" in links[0]
                                )
                            )
                        )
                        else "item",
                    }
                    if links and links[0].startswith("<"):
